...
func (b *broadcaster) drainerSend(dest uint64, send *sendRequest, successLastTime bool) bool {
	...
	if err != nil {
		if successLastTime {
			logger.Warn("could not get handle for replica %d", dest)
		}
		send.done <- false
		return false
	}
	...
	if err != nil {
		if successLastTime {
			logger.Warn("could not send to replica %d: %v", dest, err)
		}
		send.done <- false
		return false
	}
...
func (b *broadcaster) drainer(dest uint64) {
	...
	if !exsit {
		logger.Warn("could not get message channel for replica %d", dest)
		return
	}
