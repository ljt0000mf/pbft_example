...
func newObcBatch(id uint64, config *viper.Viper, stack consensus.Stack) *obcBatch {
	...
	logger.Info("PBFT Batch size = %d", op.batchSize)
	logger.Info("PBFT Batch timeout = %v", op.batchTimeout)

	if op.batchTimeout >= op.pbft.requestTimeout {
		op.pbft.requestTimeout = 3 * op.batchTimeout / 2
		logger.Warn("Configured request timeout must be greater than batch timeout, setting to %v", op.pbft.requestTimeout)
	}
	...
	if op.pbft.requestTimeout >= op.pbft.nullRequestTimeout && op.pbft.nullRequestTimeout != 0 {
		...
		logger.Warn("Configured null request timeout must be greater than request timeout, setting to %v", op.pbft.nullRequestTimeout)
	}
...
func (op *obcBatch) execute(seqNo uint64, reqBatch *RequestBatch) {
	...
	for _, req := range reqBatch.GetBatch() {
		...
		if err := proto.Unmarshal(req.Payload, tx); err != nil {
			logger.Warn("Batch replica %d could not unmarshal transaction %s", op.pbft.id, err)
			continue
		}
		logger.Debug("Batch replica %d executing request with transaction %s from outstandingReqs, seqNo=%d", op.pbft.id, tx.Txid, seqNo)
		if outstanding, pending := op.reqStore.remove(req); !outstanding || !pending {
			logger.Debug("Batch replica %d missing transaction %s outstanding=%v, pending=%v", op.pbft.id, tx.Txid, outstanding, pending)
		}
		txs = append(txs, tx)
		op.deduplicator.Execute(req)
	}
	meta, _ := proto.Marshal(&Metadata{seqNo})
	logger.Debug("Batch replica %d received exec for seqNo %d containing %d transactions", op.pbft.id, seqNo, len(txs))
...
func (op *obcBatch) leaderProcReq(req *Request) events.Event {
	// XXX check req sig
	digest := hash(req)
	logger.Debug("Batch primary %d queueing new request %s", op.pbft.id, digest)
...
func (op *obcBatch) sendBatch() events.Event {
	...
	op.batchStore = nil
	logger.Info("Creating batch with %d requests", len(reqBatch.Batch))
	return reqBatch
}
...
func (op *obcBatch) processMessage(ocMsg *pb.Message, senderHandle *pb.PeerID) events.Event {
	...
	if req := batchMsg.GetRequest(); req != nil {
		if !op.deduplicator.IsNew(req) {
			logger.Warn("Replica %d ignoring request as it is too old", op.pbft.id)
			return nil
		}
...
func (op *obcBatch) logAddTxFromRequest(req *Request) {
	...
	if true {
		...
		if err != nil {
			logger.Error("Replica %d was sent a transaction which did not unmarshal: %s", op.pbft.id, err)
		} else {
			logger.Debug("Replica %d adding request from %d with transaction %s into outstandingReqs", op.pbft.id, req.ReplicaId, tx.Txid)
		}
	}
}
...
func (op *obcBatch) ProcessEvent(event events.Event) events.Event {
	...
	case committedEvent:
		logger.Debug("Replica %d received committedEvent", op.pbft.id)
	...
	case viewChangedEvent:
		...
		for i := op.pbft.h + 1; i <= op.pbft.h+op.pbft.L; i++ {
			...
			if cert.prePrepare.RequestBatch == nil {
				logger.Warn("Replica %d found a non-null prePrepare with no request batch, ignoring")
				continue
			}
...
func (op *obcBatch) startTimerIfOutstandingRequests() {
	if op.pbft.skipInProgress || op.pbft.currentExec != nil || !op.pbft.activeView {
		...
		logger.Debug("Replica %d not starting timer because skip in progress or current exec or in view change", op.pbft.id)
		return
	}

	if !op.reqStore.hasNonPending() {
		// Only start a timer if we are aware of outstanding requests
		logger.Debug("Replica %d not starting timer because all outstanding requests are pending", op.pbft.id)
		return
	}
